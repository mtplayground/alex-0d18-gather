...
type CommentActionState = "idle" | "posting" | "deleting" | "error";
      ...
      <section className="mx-auto w-full max-w-6xl px-6 py-8 sm:px-10 lg:px-14">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-6">
            ...
            <CommentThread
              eventId={event.id}
              isHost={detail.viewer_role === "host"}
              userId={user?.id ?? null}
            />
          </div>
...
function CommentThread({
  ...
  const [actionStatus, setActionStatus] = useState<CommentActionState>("idle");
  const [message, setMessage] = useState("");
  const [deletingCommentId, setDeletingCommentId] = useState("");
  const trimmedBody = body.trim();
  const canPost = trimmedBody.length > 0 && actionStatus !== "posting";
  ...
  async function postComment(event: FormEvent<HTMLFormElement>) {
    ...
    setActionStatus("posting");
    setMessage("");

    try {
      const result = await createEventComment(eventId, trimmedBody);
    ...
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <form className="mt-5 space-y-3" onSubmit={postComment}>
        <label className="block">
          <span className="text-sm font-medium text-slate-800">Add a comment</span>
          <textarea
            className="mt-2 min-h-28 w-full resize-y rounded-md border border-slate-300 bg-white px-3 py-3 text-sm leading-6 outline-none transition focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100"
            disabled={actionStatus === "posting"}
            maxLength={2_000}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-slate-500">{trimmedBody.length}/2000</p>
          <button
            ...
            type="submit"
          >
            {actionStatus === "posting" ? "Posting..." : "Post comment"}
          </button>
        </div>
      </form>
      ...
      {loadStatus === "loading" ? (
        <p className="mt-5 text-sm leading-6 text-slate-600">Loading comments.</p>
      ) : null}
      ...
      {loadStatus === "loaded" && comments.length === 0 ? (
        <div className="mt-5 rounded-md border border-dashed border-slate-300 bg-slate-50 p-4">
          ...
          <p className="mt-1 text-sm leading-6 text-slate-600">
            Start the event conversation with a quick update or question.
          </p>
        </div>
      ) : null}
      ...
      {loadStatus === "loaded" && comments.length > 0 ? (
        <ol className="mt-5 space-y-3">
          {comments.map((comment) => {
            const canDelete = isHost || comment.author_user_id === userId;
            const isDeleting =
              actionStatus === "deleting" && deletingCommentId === comment.id;
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="text-sm font-semibold text-slate-950">
                      {comment.author_user_id === userId ? "You" : "Guest"}
                    </p>
                  ...
                  {canDelete ? (
                    <button
                      className="inline-flex h-9 items-center justify-center rounded-md border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700 transition hover:border-rose-300 hover:bg-rose-50 hover:text-rose-700 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500"
